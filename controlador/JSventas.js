...
function agregarFila() {
  ...
  if (!producto || isNaN(precio) || isNaN(cantidad) || cantidad <= 0) {
    mostrarAlerta(
      "Error",
      "Ningún campo puede estar vacío o contener datos inválidos",
      "danger"
    );
    return;
  }
  ...
  if (existingRow) {
    const existingCantidad = parseInt(
      existingRow.find("td:nth-child(4) input").val(),
      10
    );
    ...
    row.append(`
          ...
          <td><input type="number" class="form-control readonly-input" value="${precio.toFixed(
            2
          )}" readonly></td>
          <td><input type="number" class="form-control readonly-input" value="${cantidad}" readonly></td>
          <td><input type="number" class="form-control readonly-input" value="${subtotal.toFixed(
            2
          )}" readonly></td>
          <td class="text-end">
              <div class="btn-group">
